    ...
    button.addEventListener('click', (e) => {
      ...
      qsa(".gallery-row[data-category='munitions']").forEach((item) => {
        item.classList.toggle(
          'hidden',
          subFilter !== 'all' && item.getAttribute('data-subCategory') !== subFilter
        )
      })
      ...
      else if (subFilter === 'mun-2') showHiddenComponent('Missile',   [-1.44, 2.15, 0.69], [0.25, -0.05, -0.03])
      else if (subFilter === 'mun-3') showHiddenComponent('Munitions', [1.8, 0.7, -0.8],    [-0.2, -0.75, 0])
  ...
  function applyFilter(filterValue) {
    ...
    galleryRows.forEach((row) => {
      ...
      if (category === 'munitions') {
        activeSubFilter = qs('.munitions-btn.active')?.getAttribute('data-subfilter') ?? 'all'
      } else if (category === 'navigation') {
        activeSubFilter = qs('.navigation-btn.active')?.getAttribute('data-subfilter') ?? 'all'
      }
